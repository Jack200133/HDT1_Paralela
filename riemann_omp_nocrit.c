// Ejercicio 5: Integral por método de Riemann con OpenMP sin critical section
...
double trapezoides(double (*func)(double), double a, double b, int n, int threads)
{
    ...
    double *suma_local = (double *)malloc(threads * sizeof(double));
...
#pragma omp parallel num_threads(threads)
    {
        ...
        suma_local[ID_thread] = func(a_local) / 2.0;

        for (int i = 1; i < n_local; i++)
        {
            suma_local[ID_thread] += func(a_local + i * h);
        }

        suma_local[ID_thread] += func(b_local) / 2.0;
    }

    for (int i = 0; i < threads; i++)
    {
        suma_global += suma_local[i];
    }
...
int main(int argc, char *argv[])
{
    ...
    printf("Con n = %d trapezoides y %d threads, nuestra aproximación de la integral de %.2f a %.2f para 2x^3 es = %.13f\n", n, threads, a, b, trapezoides(f_2x3, a, b, n, threads));
